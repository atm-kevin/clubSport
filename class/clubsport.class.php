...
class ClubSport extends SeedObject
{
    /**
     * Canceled status
     */
	...
	public static $TStatus = array(
		self::STATUS_CANCELED => 'ClubSportStatusShortCanceled'
		,self::STATUS_DRAFT => 'ClubSportStatusShortDraft'
		,self::STATUS_VALIDATED => 'ClubSportStatusShortValidated'
//		,self::STATUS_REFUSED => 'ClubSportStatusShortRefused'
//		,self::STATUS_ACCEPTED => 'ClubSportStatusShortAccepted'
	);

	/** @var string $table_element Table name in SQL */
	public $table_element = 'clubsport';

	/** @var string $element Name of the element (tip for better integration in Dolibarr: this value should be the reflection of the class name with ucfirst() function) */
	public $element = 'clubsport';
...
//        ),
    ...
    private function getNextRef()
    {
		...
		$mask = !empty($conf->global->CLUBSPORT_REF_MASK) ? $conf->global->CLUBSPORT_REF_MASK : 'MM{yy}{mm}-{0000}';
		$ref = get_next_value($db, $mask, 'clubsport', 'ref');
...
//            $this->fk_user_valid = $user->id;
    ...
    public function getNomUrl($withpicto = 0, $moreparams = '')
    {
        ...
        $result='';
        $label = '<u>' . $langs->trans("ShowClubSport") . '</u>';
        if (! empty($this->ref)) $label.= '<br><b>'.$langs->trans('Ref').':</b> '.$this->ref;

        $linkclose = '" title="'.dol_escape_htmltag($label, 1).'" class="classfortooltip">';
        $link = '<a href="'.dol_buildpath('/clubsport/card.php', 1).'?id='.$this->id.urlencode($moreparams).$linkclose;
...
//        $picto='clubsport@clubsport';
    ...
    public static function LibStatut($status, $mode)
    {
        ...
        if ($status==self::STATUS_CANCELED) { $statusType='status9'; $statusLabel=$langs->trans('ClubSportStatusCancel'); $statusLabelShort=$langs->trans('ClubSportStatusShortCancel'); }
        elseif ($status==self::STATUS_DRAFT) { $statusType='status0'; $statusLabel=$langs->trans('ClubSportStatusDraft'); $statusLabelShort=$langs->trans('ClubSportStatusShortDraft'); }
        elseif ($status==self::STATUS_VALIDATED) { $statusType='status1'; $statusLabel=$langs->trans('ClubSportStatusValidated'); $statusLabelShort=$langs->trans('ClubSportStatusShortValidate'); }
        elseif ($status==self::STATUS_REFUSED) { $statusType='status5'; $statusLabel=$langs->trans('ClubSportStatusRefused'); $statusLabelShort=$langs->trans('ClubSportStatusShortRefused'); }
        elseif ($status==self::STATUS_ACCEPTED) { $statusType='status6'; $statusLabel=$langs->trans('ClubSportStatusAccepted'); $statusLabelShort=$langs->trans('ClubSportStatusShortAccepted'); }
...
//class ClubSportDet extends SeedObject
//{
//    public $table_element = 'clubsportdet';
//
//    public $element = 'clubsportdet';
//
//
//    /**
//     * ClubSportDet constructor.
//     * @param DoliDB    $db    Database connector
//     */
//    public function __construct($db)
//    {
